...
class DataLoad(udata.Dataset):
    ...
    def __init__(self, batch_size, patch_size=256, target_dir='data', train=True, keep_range=False):
        if train:
            split = 'train'
...
class Dataset(object):
    def __init__(self, train=True, batchSize=4):
        if train:
            ...
            self.data = DataLoader(self.transformed_data, batchSize, num_workers=0, shuffle=True)
        else:
